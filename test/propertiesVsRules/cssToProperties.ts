...
const formatCssRule = (selector: string) => {
  return removeGreaterThan(removePseudoClass(selector)).replace(/\\:/g, ':').replace(/\\/g, '').substring(1)
}
...
export const cssToProperties = (inputFile: string): TailwindPropertiesJson => {
